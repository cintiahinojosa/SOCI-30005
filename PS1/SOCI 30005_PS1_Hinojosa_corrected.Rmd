---
title: "SOCI 30005-PS2-Hinojosa-corrected"
...
nals.freq <- table(nals$Education,nals$unemp)
nals.freq
```
...
### Marginals for unemployment status
Use the margin argument to prop.table function. It tells where in rows (margin=1) or in columns (margin=2) grouping variable is.  

```{r marginals unemp}
nals.marg.unemp <- prop.table(table(nals$Education,nals$unemp), margin =2)*100
nals.marg.unemp
```
...
```{r marginals edu}
nals.marg.edu <- prop.table(table(nals$Education,nals$unemp), margin =1)*100
nals.marg.edu
```
